...
Note: currently there are no tests running, `on:push` and `on:pull_request` workflows are not yet implemented, but `pull_request:types:- closed` (when the branch is merged and closed, or just closed) the app does get built and deployed.
